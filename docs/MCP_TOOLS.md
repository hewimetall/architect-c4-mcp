...
| Tool | Notes |
|------|-------|
| `bind_docs(docs_dir?)` | Привязка к каталогу `docs/` продукта; rewrite legacy `*.json` → `*.toml` |

Happy path: `uvx architect-c4 --docs /path/to/docs`, затем обычные tool calls на привязанном `docs/`.
